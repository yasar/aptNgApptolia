        ...
        function Compile(element, attrs) {
            ...
            function link(scope, element, attrs, ctrls, transcludeFn) {
                ...
                $timeout(function () {
                    ...
                    /**
                     * if we have builder assigned in the attribute,
                     * we shall notify the builder about panel is ready.
                     *
                     * there is a good use-case for this.
                     * in dtr-issue form, we need to show `copy from yesterday` button
                     * in the footer-left section. Having this feature enable us to
                     * easily insert the button. check the issue form file for the implementation.
                     */
                    if (_.get(attrs, 'aptBuilder') && attrs.aptBuilder !== 'null') {
                        var builder = _.get($window, attrs.aptBuilder);
                        NotifyingService.notify(builder.getEventName('panel.ready'));
                    }
        ...
        vm.process = function ($formController, callback) {
            ...
            function processFooter() {
                ...
                function limitless13() {
                    ...
                    if (!$footerLeft) {
                        $footerLeft = $el.find('.form-actions-left');
                    }
                    if ($footerLeft) {
                        holder.append('<span class="heading-text text-semibold panel-footer-left">' + $footerLeft.html() + '</span>');
                    }
                    ...
                    if ($footer) {
                        ...
                        var $innerHolder = $('<div class="heading-btn pull-right panel-footer-right"></div>');
