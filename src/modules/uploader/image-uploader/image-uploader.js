...
;(function () {
    ...
    angular
        .module('apt.uploader')
        .directive('aptImageUploader', [
            'Restangular', '$uibModal', '$rootScope', '$timeout', 'aptTempl',
            function (Restangular, $modal, $root, $timeout, Templ) {
                return {
                    ...
                    replace    : true,
                    controller : [
                        '$scope', 'Restangular', 'aptUtils', 'Lightbox', 'aptTempl',
                        function ($scope, Restangular, aptUtils, Lightbox, aptTempl) {
                            $scope.state = {
                                is_modified: false,
                                is_uploaded: false
                            };
                            ...
                            if (_.has($scope.options, 'data.image_table')) {
                                $scope.settings.image_table = $scope.options.data.image_table;
                            }
                            else if ($scope.options.dbcfg_ref) {
                                $scope.settings.image_table = '`' + aptTempl.appConfig.package + '.' + $scope.options.dbcfg_ref + '_image`';
                            }
                            ...
                            $scope.editDescription = function () {
                                var modalInstance = $modal.open({
                                    ...
                                    controller : [
                                        '$scope', 'description', 'title', 'Restangular',
                            ...
                            $scope.upload = function () {
                                ...
                                settings.data.type = $scope.model.type;
                                ...
                                Restangular.all('/system/image/upload').post(settings).then(function (data) {
                                    ...
                                    $scope.uploadFile        = data.upload_path + '/' + data.filename;
                                    $scope.state.is_uploaded = true;
                                    $scope.state.is_modified = false;
                            ...
                            $scope.openLightboxModal = function () {
                                ...
                                Lightbox.openModal([$scope.uploadFileOrig], 0);
                            };
